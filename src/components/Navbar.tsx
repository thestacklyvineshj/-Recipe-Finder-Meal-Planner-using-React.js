...
import { ChefHat, Heart, Calendar, Moon, Sun, Menu, X, Library } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { useApp } from '../context/AppContext';
import { MEAL_SLOTS, DAYS_OF_WEEK } from '../utils/constants';

export const Navbar: React.FC = () => {
  const { theme, toggleTheme, favourites, mealPlan } = useApp();
    ...
    <header className="sticky top-0 z-40 w-full bg-white/90 dark:bg-zinc-900/95 border-b border-zinc-200/50 dark:border-zinc-800/60 backdrop-blur-md shadow-sm" id="main-navigation-navbar">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Theme Mode Option Selectors & Mobile Hamburger toggles */}
          <div className="flex items-center gap-3">
            {/* Theme Toggle Button */}
            <button
              onClick={toggleTheme}
              className="p-2.5 rounded-xl border border-zinc-200/50 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-650 hover:text-amber-500 dark:text-zinc-300 dark:hover:text-amber-300 shadow-sm transition hover:scale-[1.03] active:scale-[0.98] cursor-pointer"
